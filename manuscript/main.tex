...
\]
The global dimensions below are over $\F_q$, whereas component dimensions are over $K_s$.
...
\begin{proof}
The leading coefficient after reversal is $\rho_{s,k}(f_0)$, so the displayed normalization makes the reciprocal monic. Reversal and the coefficient automorphism commute with products up to the scalar normalization, which proves multiplicativity. In a finite field the ordinary normalized reciprocal preserves irreducibility, and applying the field automorphism preserves it as well. Applying the inverse construction with $\sigma_{s,k}$ returns the original monic polynomial.
\end{proof}
...
\begin{proof}
The coefficient vectors of the shifts of $G$ and the shifts of $H^*$ are orthogonal: after reversing the coefficient order in $H$ and using $GH=M$, every coefficient pairing is a coefficient of a multiple of $x^n-a$ and hence vanishes in the Euclidean pairing. The shifts of $H^*$ span a space of dimension $\deg G$, which is the codimension of $\langle G\rangle$. Thus the orthogonal space is exactly $\langle H^*\rangle$. The boundary shift changes the scalar from $a$ to $a^{-1}$, giving the stated constacyclic quotient.\end{proof}
...
\[
\begin{aligned}
\operatorname{supp}\Hull_k(C_s(J_s))
...
Let $O\in\Os$ be a cycle of length $a_O$, indexed by
\[
...
\]
which is the displayed monomial after cyclic reindexing. This is the standard closed-walk trace mechanism; the hull-specific content is the preceding identification of the boundary statistic.\end{proof}
...
Assume the hypotheses of Sections 2--5, including square-free affine decomposition, $\gcd(n,p)=1$, simple roots, fixed labeled components, finite-field-linear component codes, equal factor degree within each reciprocal orbit, code-first duality, and
\[
\lambda_s^{1+p^{d_s-k}}=1\qquad(1\leq s\leq N).
\]
For every orbit $O\in\Os$, let $a_O$ be its length and $w_O=m_s d_O$ its global weight. For a labeled factor selection $J=(J_s)_s$, let
\[
K(C(J))=\sum_{s,O,i}w_O(1-\varepsilon_{O,i}),
\qquad
H_k(C(J))=\sum_{s,O,i}w_O\varepsilon_{O,i}(1-\varepsilon_{O,i+1}).
\]
Then the exact joint generating polynomial is
\[
\begin{aligned}
\Epoly(u,z)
...
\begin{proof}
The factor-selection lemma gives a Cartesian product of independent binary choices over the labeled factors. The hull-support theorem identifies the global hull dimension with the sum of the orbit contributions $w_Ob_O$. The local trace identity enumerates the choices on each orbit with their code-dimension and hull-dimension weights. Since different orbits and components are independent coordinates in the selection space and both dimensions are additive, multiplication of the local trace polynomials gives the first equality and the product formula. Coefficient extraction has the stated labeled interpretation.\end{proof}
...
Use the uniform distribution on the finite labeled selection space. For an orbit of length $a\geq2$, the indicator $X_i=\varepsilon_i(1-\varepsilon_{i+1})$ has expectation $1/4$, while a length-one orbit contributes zero. Independence across orbits gives
\[
\mathbb E[H_k]
...
The reproducible commands and complete outputs are retained in the repository. The listed cases are the fully specified cases approved by the prior manuscript specification:
...
The internal validation artifact N1 remains `UNSPECIFIED — CANNOT VALIDATE` and is excluded from the manuscript. No missing twist, factorization, orbit decomposition, histogram, or numerical output is inferred.
...
The finite computations validate the stated F4, F8, F16, compatible-twist, and N2 instances through independent routes. They do not replace the conditional proofs. The work does not address repeated roots, incompatible same-factor-set transfer, equivalence-class enumeration, Burnside/Pólya quotients, or unconditional quantum parameters. The literature review records narrower exact enumeration results and standard combinatorial tools, while retaining the unresolved final/corrected source checks and making no priority claim.
