...
Here p^k=1, so
    lambda^(1+p^k)=omega^2 != 1.
The direct dual is checked to be lambda^(-p^k)=omega^2-constacyclic,
...
def main():
    ...
    check_sharp = normalized_galois_reciprocal(F, check, SIGMA)
    ...
    lambda_prime = F.inv(F.pow(LAMBDA, SIGMA))
    assert lambda_prime != LAMBDA
    assert F.pow(LAMBDA, 1 + SIGMA) != 1
    ...
    print("  q=4, n=5, lambda=omega, k=0, p^k=1")
    print("  lambda^(1+p^k)=", F.pow(LAMBDA, 1 + SIGMA), "!= 1")
    print("  lambda^(-p^k)=", lambda_prime, "!= lambda")
    print("  direct dual equals <h^(#)>:", direct_dual == expected_dual)
    print("  direct dual is lambda^(-p^k)-constacyclic:",
