...
The script checks:
  ...
  * the k=1 Galois/Hermitian factor action;
...
def normalized_galois_reciprocal(f, sigma_exponent=2):
    """f -> f_0^{-p^k} sum_i f_i^(p^k) x^(d-i), for k=1 over F_4."""
    degree = len(f) - 1
    constant = square(f[0]) if sigma_exponent == 2 else f[0]
    out = [0] * (degree + 1)
    for i, coefficient in enumerate(f):
        image = square(coefficient) if sigma_exponent == 2 else coefficient
...
def galois_dot(x, y):
    # k=1, so the second argument is squared.
    value = 0
    for a, b in zip(x, y):
        value ^= mul(a, square(b))
    return value
...
def hull_dimension_for_selection(factors, selected, length=5):
    ...
    hull = {
        vector
        for vector in code
        if all(galois_dot(vector, row) == 0 for row in rows)
    }
    size = len(hull)
    dimension = 0
    while size > 1:
...
def main():
    ...
    images = [normalized_galois_reciprocal(factor) for factor in quadratic_factors]
    ...
    assert normalized_galois_reciprocal(linear_factor) == linear_factor
