...
def normalized_galois_reciprocal(f, rho=2):
    """f -> f_0^{-rho} sum_i f_i^rho x^(d-i), for k=1 over F_4."""
    degree = len(f) - 1
    constant = square(f[0]) if rho == 2 else f[0]
    out = [0] * (degree + 1)
    for i, coefficient in enumerate(f):
        image = square(coefficient) if rho == 2 else coefficient
