...
def normalized_galois_reciprocal(F, f, rho):
    """f_0^{-rho} sum_i f_i^rho x^(d-i), for monic f."""
    degree = len(f) - 1
    constant_image = F.frobenius(f[0], rho)
    scale = F.inv(constant_image)
    result = [0] * (degree + 1)
    for i, coefficient in enumerate(f):
        image = F.frobenius(coefficient, rho)
