...
The publisher/abstract audit confirms the broad finite-field and affine-algebra hull scope. The accessible arXiv preprint also displays a candidate-first definition `C^{\perp_k}={\alpha:\langle\alpha,c\rangle_k=0}` while displaying an inverse-Frobenius reciprocal. The present blueprint deliberately freezes the code-first definition `\langle c,x\rangle_k=0`; exact source theorem labels, displayed exponents, convention translation, and overlap with the present transfer enumerator remain **THEOREM-LEVEL: VERIFY BEFORE MANUSCRIPT FINALIZATION**. The present work proves its formulas independently under the declared code-first convention and imports no source formula without reconciling the argument order.
...
| N1 | `q=16`, `n=15`, 32,768 proposed selections; missing `k`, twist, factorization, and orbit data | `2^{15}` | **UNSPECIFIED — CANNOT VALIDATE** |
