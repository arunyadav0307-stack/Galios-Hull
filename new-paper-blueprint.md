...
The N2-A validator compares the direct dual with both reciprocal exponents and records A=B for all 32 selections. The N2-B validator independently checks an incompatible nontrivial twist with `m_s=2`, including the predicted dual twist and failure under the original twist, without transfer-matrix enumeration.
...
Thus the validators transform code rows by `\rho_{s,k}` before solving ordinary nullspace equations. They compare that direct dual with the generator from the normalized reciprocal using the same `\rho_{s,k}` exponent, and where applicable with the predicted twist `\lambda_s^{-p^{e m_s-k}}`. These finite runs validate only their listed parameters; the general identities still require the proofs in Sections 4–9.
...
The two exponents happen to coincide in this pilot, so it cannot separate the two convention candidates. The validator nevertheless uses the code-in-the-first-slot definition and the principal `\rho` implementation. Let `\omega^2+\omega+1=0`. In characteristic two,
...
and `\sigma` and `\rho` induce the same identity automorphism on `\mathbb F_4`. Therefore this case cannot resolve which exponent is principal. It is retained only as a basic incompatible-twist sanity diagnostic.

The twist calculation is
...
basic incompatible-twist sanity diagnostic only:
  ...
  sigma == rho as automorphisms on F_4: True
  lambda^(1+rho)= 3 != 1
  lambda^(-rho)= 3 != lambda
...
This is **DIAGNOSTIC ONLY**, not a convention-resolution validation. The extension-field N2-B diagnostic below is the convention-separating incompatible check.
...
### 10.7 N2-A: compatible extension-field convention separation
...
- **C. Incorrect comparator:** generate the candidate dual with exponent `2`, namely the explicitly stale `p^k` formula. This comparator is not a principal convention.

The compatible predicted dual twist is
...
N2 extension-field convention validation:
  ...
  compatible predicted dual twist lambda^(-rho): 1
...
This is a finite **COMPUTATIONAL VALIDATION** of the declared orientation and does not replace the general dual-generator proof.

### 10.8 N2-B: incompatible extension-field diagnostic, no transfer enumeration

Use the separately checked extension-field parameters
...
with a primitive element `\alpha` of `\mathbb F_{16}` and
...
Applying the convention-derived incompatible formula gives the predicted dual twist

\[
\lambda^{-\rho}=12
\]

in the validator's `\mathbb F_{16}` integer representation. The validator exhaustively checks all eight factor selections by direct dual equations, the `\rho` reciprocal generator, and constacyclicity under the predicted twist. It also checks all six proper nonzero selections against the original `\lambda` twist. It does **not** enumerate a transfer matrix, because the dual factors are outside the compatible same-factor-set theorem.
...
```text
N2-B incompatible-twist diagnostic:
  K=F_16=F_(4^2), q=4, p=2, e=2, m_s=2, k=1, n=3
  lambda=alpha^3 (order 5), sigma=2, rho=8
  sigma(alpha) != rho(alpha): True
  factorization: (x+alpha)(x+alpha^6)(x+alpha^11)
  lambda^(1+rho) != 1: True
  predicted dual twist lambda^(-rho): 12
  direct dual checks: 8 of 8
  direct dual == <h^(#_rho)>: 8 of 8
  dual constacyclic under predicted twist: 8 of 8
  proper nonzero codes failing original twist: 6 of 6
  example proper selection mask failing original twist: 1
  No transfer-matrix enumeration was attempted.
PASS: incompatible extension-field twist diagnosed without enumeration
```

This is a **COMPUTATIONAL VALIDATION** of the incompatible-twist formula for this finite extension example and a **DIAGNOSTIC** outside the compatible theorem. It is not a transfer-matrix result.
...
| Basic incompatible diagnostic | `q=4`, `n=5`, `\lambda=\omega`, `k=0`; `\sigma=\rho` as automorphisms | one selected code; no enumerator | **DIAGNOSTIC ONLY**; executed sanity check |
| N1 | `q=16`, `n=15`, 32,768 selections | `2^{15}` | **VERIFY BEFORE MANUSCRIPT FINALIZATION** |
| N2-A | `K=F_{16}=F_{4^2}`, `\sigma=2`, `\rho=8`, `n=5`, `\lambda=1` | all `32` selections and A/B/C comparison | **COMPUTATIONAL VALIDATION**: PASS; A=B 32/32, incorrect comparator A=C 8/32 |
| N2-B | `K=F_{16}=F_{4^2}`, `k=1`, `n=3`, `\lambda=\alpha^3`; incompatible | all `8` selections; six proper nonzero original-twist checks; no transfer enumeration | **COMPUTATIONAL VALIDATION**: PASS; **DIAGNOSTIC** outside theorem |
...
- N2-A in Section 10.7, including A=B under `\rho` and the explicitly incorrect `p^k` comparator;
- N2-B in Section 10.8, including the predicted incompatible twist and failure under the original twist, with no transfer-matrix enumeration;
...
9. **Examples and exhaustive validation:** pilot, long orbit, extension component, compatible nontrivial twist, N1, N2-A, N2-B, and incompatible diagnostics.
...
- [ ] N2-A confirms A=B for all 32 selections under `\rho=8` and records that the explicitly incorrect `p^k=2` comparator differs on 24.
- [ ] N2-B uses `K=\mathbb F_{16}`, `k=1`, `n=3`, `\lambda=\alpha^3`, checks the predicted twist, checks failure under the original twist, and uses no transfer enumeration.
...
8. Report N2-A's A=B/C comparison and N2-B's incompatible predicted-twist checks; keep N1 marked `VERIFY BEFORE MANUSCRIPT FINALIZATION` until its parameters and output exist.
...
- **Validation corrections:** Rebuilt the validators under the inverse convention, added direct dual-generator checks, verified N2-A with A=B for all 32 `\mathbb F_{16}` selections and the incorrect `p^k=2` comparator at 8/32, added checked N2-B with predicted-twist and original-twist checks, labeled the old `F_4`, `k=0` case `DIAGNOSTIC ONLY`, and retained N1 as unverified.

## Final Consistency Status

- **Convention:** Resolved from `\langle x,y\rangle_{s,k}=\sum_i x_i y_i^{p^k}` together with the declared code-first dual slot `\langle c,x\rangle_{s,k}=0`. The derivation applies `\rho_{s,k}=\sigma_{s,k}^{-1}` to the equations, so the principal reciprocal, root action, compatibility condition, dual generator, factor permutation, hull support, and incompatible dual twist all use `p^{e m_s-k}`. The `p^k` reciprocal appears only as the explicitly incorrect N2-A comparator.
- **Compatible theorem scope:** The transfer-matrix and same-factor-set orbit statements apply only under componentwise `\lambda_s^{1+p^{e m_s-k}}=1`, square-free affine decomposition, `\gcd(n,p)=1`, and labeled factor selections. Incompatible twists are not transferred into that theorem.
- **Checked computational results:** `validate_pilot.py`, `validate_long_orbit_examples.py`, `validate_nontrivial_constacyclic.py`, `validate_n2_extension_convention.py`, `diagnose_incompatible_twist.py`, and `diagnose_n2_incompatible_twist.py` were executed with `PYTHONDONTWRITEBYTECODE=1`. The long-orbit direct generator checks are 128/128 and 32/32; the compatible nontrivial-twist checks are 8/8; N2-A has A=B 32/32 and the incorrect comparator A=C 8/32; N2-B has direct, reciprocal-generator, predicted-twist, and original-twist-failure checks of 8/8, 8/8, 8/8, and 6/6. The basic `F_4`, `k=0` check is **DIAGNOSTIC ONLY** because `\sigma` and `\rho` coincide there. No N2-B transfer enumeration was attempted.
- **Unresolved status:** N1 remains exactly **VERIFY BEFORE MANUSCRIPT FINALIZATION**. Literature records marked metadata-only, DOI-unverified, or theorem-level unresolved remain so until directly checked. No unexecuted computation is labeled `PASS`.
- **Scope and claims:** Counts are for distinct labeled codes, not equivalence classes. Burnside/equivalence enumeration and quantum applications remain conditional or future work; no minimum distance is inferred from hull dimension, no unsupported priority claim is made, and no unconditional complexity claim is made.
